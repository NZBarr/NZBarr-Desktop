...
function analyzeNzbWithDownload($nzbPath, $nntp, $debugNzb = false, $downloadAll = false, $keepFiles = false, $maxDownloadMb = 50) {
    ...
    if (!empty($videoFiles)) {
        echo "   🎬 Found video files in NZB: " . implode(', ', $videoFiles) . "\n";
    } else {
    ...
    if (!empty($filesToTry)) {
        ...
        if ($isArchive) {
            ...
            // Download data (only for non-RAR files)
            ...
            if ($videoData) {
                ...
                // Save the already-decoded data directly
                ...
                if (file_exists($actualDecodedFile) && filesize($actualDecodedFile) > 1000) {
                    ...
                    // Check if we got proper MediaInfo
                    if ($mediainfo && strlen($mediainfo) > 500 && (strpos($mediainfo, 'Video') !== false || strpos($mediainfo, 'Duration') !== false)) {
                        $result['mediainfo'] = $mediainfo;
    ...
    if (!empty($archiveFiles)) {
        ...
        if (!$result['password']) {
            echo "   🔍 Checking archives for password protection...\n";
...
function downloadFileFromNzb($nzbContent, $filename, $nntp, $maxBytes = 0) {
    // Find ALL file blocks matching this filename in the NZB
    $pattern = '/<file[^>]*subject="[^"]*' . preg_quote($filename, '/') . '(?:[^"]*)"[^>]*>.*?<\/file>/is';
    
    if (!preg_match_all($pattern, $nzbContent, $fileBlocks)) {
    ...
    // Extract all segment message IDs from ALL matching file blocks and sort them properly
    $segmentsWithNumbers = [];
    foreach ($fileBlocks[0] as $block) {
        preg_match_all('/<segment[^>]*bytes="[^"]*"[^>]*number="(\d+)"[^>]*>([^<]+)<\/segment>|<segment[^>]*number="(\d+)"[^>]*>([^<]+)<\/segment>/is', $block, $segments);
        // The regex above has 4 capture groups because attributes can be in different order
        foreach ($segments[0] as $idx => $fullMatch) {
            if (preg_match('/number="(\d+)"/i', $fullMatch, $numMatch)) {
                $num = (int)$numMatch[1];
                $msgId = preg_replace('/<[^>]+>/', '', $fullMatch);
                $segmentsWithNumbers[$num] = trim($msgId);
            }
        }
    }
    
    if (empty($segmentsWithNumbers)) {
        return null;
    }
    
    // Sort by segment number key
    ksort($segmentsWithNumbers);
    $allSegments = array_values($segmentsWithNumbers);
    
    $data = '';
    $bytesDownloaded = 0;
    $maxSegments = $maxBytes > 0 ? ceil($maxBytes / 700000) : count($allSegments); // Limit segments if maxBytes set
    if ($maxSegments < 50 && $maxBytes > 0) $maxSegments = 50; // Ensure we get enough for a small limit
    
    echo "      📥 Downloading from usenet (" . count($allSegments) . " segments, max $maxSegments)...\n";
    ...
    $nntp->setTimeout(10);
        ...
        foreach ($allSegments as $i => $messageId) {
            $segmentsTried++;
...
function downloadCompleteFileFromNzb($nzbContent, $filename, $nntp, $maxBytes = 0) {
    ...
    if (!preg_match_all($pattern, $nzbContent, $fileBlocks)) {
        ...
        $pattern = '/<file[^>]*subject="[^"]*' . preg_quote(basename($cleanFilename), '/') . '(?:[^"]*)"[^>]*>.*?<\/file>/is';
        if (!preg_match_all($pattern, $nzbContent, $fileBlocks)) {
            echo "      ❌ Could not find file block for: $cleanFilename\n";
            return null;
        }
    }
    
    // Debug: show what we matched (first one)
    preg_match('/subject="([^"]+)"/i', $fileBlocks[0][0], $subjectMatch);
    if ($subjectMatch) {
        echo "      🔍 Matched subject: {$subjectMatch[1]}\n";
    }
    
    // Extract all segment message IDs from ALL matching file blocks
    $allSegments = [];
    foreach ($fileBlocks[0] as $block) {
        preg_match_all('/<segment[^>]*>([^<]+)<\/segment>/is', $block, $segments);
        if (!empty($segments[1])) {
    ...
    if (empty($allSegments)) {
        echo "      ❌ No segments found for: $cleanFilename\n";
        return null;
    }
    ...
    if ($maxBytes > 0) {
        ...
        if ($maxSegments < 50) $maxSegments = 50; // Ensure we get enough for a small limit
