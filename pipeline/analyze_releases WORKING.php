...
function downloadCompleteFileFromNzb($nzbContent, $filename, $nntp, $maxBytes = 0) {
    ...
    preg_match('/subject="([^"]+)"/i', $fileBlock[0], $subjectMatch);
    if ($subjectMatch) {
    ...
    $totalSegments = count($segments[1]);
    
    if ($maxBytes > 0) {
        // Average yEnc segment is ~700KB.
        $maxSegments = ceil($maxBytes / 700000);
        if ($totalSegments > $maxSegments) {
            $segments[1] = array_slice($segments[1], 0, $maxSegments);
            $totalSegments = count($segments[1]);
    ...
    $chunks = array_chunk($segments[1], $chunkSize);
