...
function parseMediaInfo($mediainfo) {
    ...
    } elseif (preg_match('/Format\s*:\s*(E-AC-3\s*JOC|E-AC-3)/i', $mediainfo, $m)) {
        // Check if it's Dolby Atmos (E-AC-3 JOC = Joint Object Coding)
        if (preg_match('/Commercial name.*?(Dolby Atmos|with Dolby Atmos)/i', $mediainfo)) {
            $data['audio_codec'] = 'DD+ Atmos';
        } else {
            $data['audio_codec'] = 'DD+';
        }
    } elseif (preg_match('/Format\s*:\s*(AC-3|Dolby)/i', $mediainfo, $m)) {
    ...
    // Also check Complete name (filename) for DDPA or DD5.1.Atmos patterns
    if (preg_match('/Complete name.*?[-_.\s](DDPA5\.1|DDPA|DDP5\.1\.Atmos|DD\+?\s*Atmos|DD5\.1\s*Atmos|DD5\.1[-_.\s]Atmos)[-._\s]/i', $mediainfo, $m)) {
        $data['audio_codec'] = 'DD+ Atmos';
    }
